...
export const buildDropdown = (recipes, type, list, filtredInput) => {
  ...
  allLi.forEach((li) => {
    li.addEventListener("click", (e) => {
      ...
      tag.setAttribute("id", "tagCreated");
      tag.id = li.id;
  ...
  //     tagClose[i].addEventListener('click', function () {
  //       this.parentNode.remove();

  // removeTag();
  // let tagClose = document.getElementById("cross");
  // let tagHtmlDOM = document.getElementById("tagCreated");

  // tagHtmlDOM.addEventListener("click", function close() {
  //   for (var i = 0; i < tagClose.length; i++) {
  //     tagClose[i].addEventListener("click", function () {
  //       this.parentNode.remove();
  //     });
